...
% initialize group given sensing system 1
% for i=1:length(sensor_detection{1})
%     group{end+1} = sensor_detection{1}{i}.ID;
% end

for i = 1:length(r)-1 % i = 1 to num_sensing-1
    for j = i+1:length(r) % i+1 to num_sensing
        [assignment, unassigned_1, unassigned_2] = TracksInPair(sensor_detection{i}, sensor_detection{j}, threshold);
        ...
        for m = 1:size(ID_matrix,2) 
            flag = ismember(ID_matrix(1,m), cell2mat(group));
                if flag
                    idx = FindGroupIdx(group, ID_matrix(1,m));
                    group{idx}(end+1) = ID_matrix(2,m);
                    group{idx} = unique(group{idx});
                else
...
function [assignment, unassigned_1, unassigned_2] = TracksInPair(detection_1, detection_2, threshold)
